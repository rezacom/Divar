import React, { useState, useEffect, useContext } from "react";
import { makeStyles } from "@material-ui/core/styles";
import Grid from "@material-ui/core/Grid";
import SearchMain from "../searchMain";
import Tags from "../tags";
import Cart from "../cart";
import load from "../../load.svg";
import CityContext from "../context/cityContext";
import houseIcon from "../../images/icons/house.svg";
import carIcon from "../../images/icons/car.svg";
import deviceIcon from "../../images/icons/electronic-devices.svg";
import homeApplianceIcon from "../../images/icons/home-appliance.svg";
import serviceIcon from "../../images/icons/service.svg";
import entertainmentIcon from "../../images/icons/entertainment.svg";
import socialIcon from "../../images/icons/social.svg";
import businessIcon from "../../images/icons/business.svg";
import recruitmentIcon from "../../images/icons/recruitment.svg";
import personalStuffIcon from "../../images/icons/personal-stuff.svg";
import { Link } from "react-router-dom";

const useStyles = makeStyles(theme => ({
  root: {
    flexGrow: 1
  },
  paper: {
    padding: theme.spacing(2),
    textAlign: "center",
    color: theme.palette.text.secondary
  },
  marginTop: {
    marginTop: 30
  },
  imgLoad: {}
}));

export default function CenteredGrid() {
  const classes = useStyles();
  const city_context = useContext(CityContext);

  // const [cityApi, setCityApi] = useState('tehran')
  const [categoryId, setCategoryId] = useState();
  const [searchFlag, setSearchFlag] = useState(false);
  const [itemResultState, setItemResultState] = useState(null);
  const [cityId, setCityId] = useState(city_context)

  const [apiResponse, setApiResponse] = useState({
    error: null,
    isLoaded: true,
    data: null
  });

  const [apiUrl, setApiUrl] = useState({
    url: "https://api.divar.ir/v8/web-search/",
    cityId: city_context ? city_context : "tehran"
  });

  console.log('city context before:',city_context)
  useEffect(() => {

    if (city_context) {
      setApiUrl({
        url: "https://api.divar.ir/v8/web-search/",
        cityId: city_context
      });
    }
    console.log('city context after:',city_context)

    
    fetch(`${apiUrl.url}${apiUrl.cityId}`)
    
      .then(res => res.json())
      .then(res => {
        setItemResultState(res.widget_list);

        setApiResponse({
          error: null,
          isLoaded: false,
          data: res
        });
        console.log(res);
      })
      .catch(err => {
        setApiResponse({
          error: err,
          isLoaded: true,
          data: null
        });
        setItemResultState(null);
      });
  }, [city_context, categoryId]);

  

  function handleTagId(id) {
    setCategoryId(id);
  }

  function handleValueSearch(value) {
    if (value)
      setSearchFlag({
        flag: true,
        value: value
      });
    else setSearchFlag(false);
  }

  return (
    <div className={classes.root}>
      <Grid container spacing={3} className={classes.marginTop}>
        <Grid item xs={3}>
          <aside className="sidebar">
            <div className="sidebox">
              <ul className="sidebox__nav">
                <li className="sidebox__nav-title">دسته بندی ها</li>

                <Link to="/real-estate">
                  <li className="sidebox__nav-item">
                    <img src={houseIcon} alt="" />
                    املاک
                  </li>
                </Link>
                <Link to="/vehicles">
                  <li className="sidebox__nav-item">
                    <img src={carIcon} alt="" />
                    وسایل نقلیه
                  </li>
                </Link>
                <Link to="/electronic-devices">
                  <li className="sidebox__nav-item">
                    <img src={deviceIcon} alt="" />
                    لوازم الکترونیکی
                  </li>
                </Link>

                <Link to="/home-and-kitchen">
                  <li className="sidebox__nav-item">
                    <img src={homeApplianceIcon} alt="" />
                    مربوط به خانه
                  </li>
                </Link>
                <Link to="/services">
                  <li className="sidebox__nav-item">
                    <img src={serviceIcon} alt="" />
                    خدمات
                  </li>
                </Link>
                <Link to="/personal-goods">
                  <li className="sidebox__nav-item">
                    <img src={personalStuffIcon} alt="" />
                    وسایل شخصی
                  </li>
                </Link>
                <Link to="/entertainment">
                  <li className="sidebox__nav-item">
                    <img src={entertainmentIcon} alt="" />
                    سرگرمی و فراغت
                  </li>
                </Link>
                <Link to="/social-services">
                  <li className="sidebox__nav-item">
                    <img src={socialIcon} alt="" />
                    اجتماعی
                  </li>
                </Link>
                <Link to="/businesses">
                  <li className="sidebox__nav-item">
                    <img src={businessIcon} alt="" />
                    برای کسب و کار
                  </li>
                </Link>
                <Link to="/jobs">
                  <li className="sidebox__nav-item">
                    <img src={recruitmentIcon} alt="" />
                    استخدام و کاریابی
                  </li>
                </Link>
              </ul>
            </div>
          </aside>
        </Grid>
        <Grid item xs={9}>
          <SearchMain catName={"همه اگهی ها"} valueSearch={handleValueSearch} />
          <div className="tag-container" id="parentTagList">
            {!apiResponse.isLoaded
              ? apiResponse.data.suggestion_list.map((item, index) => (
                  <Tags
                    title={item.displayed_text}
                    key={index}
                    handleTagId={handleTagId}
                    id={item.value.category.value}
                  />
                ))
              : ""}
          </div>

          <div className="cart-container-p">
            {!apiResponse.isLoaded && apiResponse.data.seo_details.title
              ? apiResponse.data.seo_details.title
              : ""}
          </div>

          <div className="cart-container">
            {!searchFlag.flag ? (
              !apiResponse.isLoaded ? (
                apiResponse.data.widget_list.map((item, index) => (
                  <Link
                    to={{
                      pathname: "item",
                      search: `?${item.data.title}-${item.data.city}`,
                      state: { item },
                    }}
                    className="cart-box"
                  >
                    <Cart key={index} cartDate={item} />
                  </Link>
                ))
              ) : (
                <div className="loading-container">
                  <img src={load} className="loading-container__img" />
                </div>
              )
            ) : (
              itemResultState
                .filter(item => item.data.title.includes(searchFlag.value))
                // .filter(item => console.log(item.data.title))
                .map((item, index) => (
                  <Link
                    to={{
                      pathname: "item",
                      search: `?-${item.data.title}-${item.data.city}`,
                      state: { item },
                    }}
                    className="cart-box"
                  >
                    <Cart key={index} cartDate={item} />
                  </Link>
                ))
            )}
          </div>
        </Grid>
      </Grid>
    </div>
  );
}
